...
struct UserPostCardView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: .medium) {
            #warning("Upload User Avatars to Firebase & fetch them")
            Image(uiImage: /*(status) ?*/ userProvidedImage("user_Image") ?? UIImage.init(named: "Logo Apple")! /*: UIImage.init(named: "usericon")!*/)
                .resizable()
        ...
        .overlay(alignment: .topTrailing, content: {
            ...
            if let user_id = userVM.userProfile?.userUID {
    ...
    @ViewBuilder func InternalPostInteraction() -> some View {
        HStack(spacing: .small) {
            Button {
                ...
                if let userID = userVM.userProfile?.userUID {
                    if post.upvoteIDs.contains(userID) {
                        Image(systemName: "hand.thumbsup.fill")
                    } else {
                        Image(systemName: "hand.thumbsup")
                    }
                } else {
                    Image(systemName: "hand.thumbsup") // Default to thumbs up icon if user ID is nil
                }
            }
            ...
            Button {
                ...
                if let userID = userVM.userProfile?.userUID {
                    ...
                    Image(systemName: "hand.thumbsdown") // Default to thumbs down icon if user ID is nil
                }
            }
    ...
    func upvote() {
        Task {
            ...
            guard let userUID = userVM.userProfile?.userUID else {
